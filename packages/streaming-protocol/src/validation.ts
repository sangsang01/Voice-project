...
import type { ClientControl, NativeBackend, ServerMessage } from "./messages.js";

const NATIVE_BACKENDS = new Set<NativeBackend>(["cpu", "cuda", "vulkan", "metal"]);
...
function requireFiniteNonnegativeInteger(value: unknown, field: string): number {
...
function validateTranscriptSegment(value: unknown): TranscriptSegment {
  ...
  const language: TranscriptSegment["language"] = {
...
function validateEngineEvent(value: unknown): EngineEvent {
  ...
  switch (value.type) {
    case "state":
      ...
      return { type: "state", sessionId, sequence, state: value.state as EngineState };
    case "segment.upsert":
      return {
        type: "segment.upsert",
        sessionId,
    ...
    case "warning":
      ...
      return {
        ...
        sessionId,
        sequence,
        code: value.code as WarningCode,
    ...
    case "error": {
      ...
      const event: Extract<EngineEvent, { type: "error" }> = {
        type: "error",
        sessionId,
        sequence,
...
export function validateServerMessage(value: unknown): ServerMessage {
  ...
  switch (value.type) {
    case "session.accepted": {
      ...
      if (typeof backend !== "string" || !NATIVE_BACKENDS.has(backend as NativeBackend)) {
      ...
      return {
        ...
        backend: backend as NativeBackend,
      };
    }
    case "audio.ack":
      return {
