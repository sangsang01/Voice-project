...
describe("CloudEngineClient", () => {
  it("rejects open() without consent and never constructs a socket", async () => {
    ...
    await expect(client.open(request, (event) => events.push(event))).rejects.toThrow();
  ...
  it("opens the socket, sends session.start, and exposes an EngineSession implementing push/stop/cancel", async () => {
    const { client, getSocket } = makeClient();
    const events: EngineEvent[] = [];
    const openPromise = client.open(request, (event) => events.push(event));
  ...
  it("forwards two PCM frames as binary sends", async () => {
    const { client, getSocket } = makeClient();
    const openPromise = client.open(request, () => {});
    getSocket().simulateOpen();
    const session = await openPromise;

    session.push({ sequence: 0, startMs: 0, samples: Int16Array.from([1, 2, 3, 4]) });
    session.push({ sequence: 1, startMs: 20, samples: Int16Array.from([5, 6, 7, 8]) });

    const binarySends = getSocket().sent.filter((chunk) => chunk instanceof ArrayBuffer);
    expect(binarySends).toHaveLength(2);
  });

  it("relays provisional and final events emitted by the server", async () => {
    const { client, getSocket } = makeClient();
    const events: EngineEvent[] = [];
    const openPromise = client.open(request, (event) => events.push(event));
    getSocket().simulateOpen();
  ...
  it("drops a frame and emits AUDIO_GAP when the socket is backed up (backpressure)", async () => {
    ...
    session.push({ sequence: 0, startMs: 0, samples: Int16Array.from([1, 2]) });
