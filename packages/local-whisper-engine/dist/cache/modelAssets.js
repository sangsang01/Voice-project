 ...
 */
export async function loadModelAsset(fileName, options = {}) {
    ...
    if (!response.ok) {
        throw new Error(`Could not load ${fileName} (HTTP ${response.status}). Run "npm run fetch-models" to download the model weights.`);
    }
