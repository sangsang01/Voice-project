...
export function createWorkerController(runtime, post, options = {}) {
    ...
    const emitState = (session, state) => {
        emit(session, { type: "state", state });
    };
    const convert = (frames) => {
    ...
    const credit = (session, frames) => {
        if (frames > 0)
            post({ type: "credit", sessionId: session.request.sessionId, frames });
    };
    const drainWindow = async (session, final) => {
    ...
    const handleMessage = async (message) => {
        switch (message.type) {
            ...
            case "push": {
                ...
                if (active.frames.length >= windowFrames)
                    await drainWindow(active, false);
                return;
            }
            case "stop": {
                if (!active || active.terminal || active.request.sessionId !== message.sessionId)
...
async function createTransformersRuntime() {
    let pipeline;
    return {
