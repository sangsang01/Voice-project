...
export async function main(env = process.env) {
    ...
    const pool = await RuntimePool.create({
        ...
        capacity,
        createHandle: () => addon.createRuntime({
    ...
    const metrics = metricsPath && metricsPath.length > 0
    ...
    console.error(`transcription server listening on ${config.bindHost}:${config.port} model=${config.modelPath} capacity=${capacity}`);
}
/** HTTP readiness: 200 only after every warm pool handle is available. */
export function handleHttpRequest(request, response, isReady) {
