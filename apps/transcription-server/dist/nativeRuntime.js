 ...
 */
export function createNativeRuntimeSession(options) {
    ...
    const raceDecode = async (audio, prompt) => {
        if (unhealthy || closed) {
        ...
        try {
            ...
            markUnhealthy();
            void decodePromise.then(() => undefined, () => undefined);
            throw error;
        }
        finally {
            if (timer !== undefined)
    ...
    return {
        push(frame) {
            if (closed || unhealthy)
                return { speechStarted: false, speechEnded: false };
