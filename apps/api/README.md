...
The workspace packages this app depends on
(`@voice/transcription-contracts`, `@voice/google-transcription-engine`)
are consumed as TypeScript source, not a prebuilt `dist`, so running the
compiled `tsc` output alone will not resolve them. Use the `start` script,
which runs the source directly through `tsx`:
